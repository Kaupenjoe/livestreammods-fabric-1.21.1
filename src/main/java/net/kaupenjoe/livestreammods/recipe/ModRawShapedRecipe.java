...
public class ModRawShapedRecipe {
    private static final int MAX_WIDTH_AND_HEIGHT = 5;
    public static final MapCodec<ModRawShapedRecipe> CODEC = ModRawShapedRecipe.Data.CODEC
            .flatXmap(
                    ModRawShapedRecipe::fromData,
                    recipe -> (DataResult)recipe.data.map(DataResult::success).orElseGet(() -> DataResult.error(() -> "Cannot encode unpacked recipe"))
            );
    public static final PacketCodec<RegistryByteBuf, ModRawShapedRecipe> PACKET_CODEC = PacketCodec.of(ModRawShapedRecipe::writeToBuf, ModRawShapedRecipe::readFromBuf);
    private final int width;
    private final int height;
    private final DefaultedList<Ingredient> ingredients;
    private final Optional<ModRawShapedRecipe.Data> data;
    private final int ingredientCount;
    private final boolean symmetrical;

    public ModRawShapedRecipe(int width, int height, DefaultedList<Ingredient> ingredients, Optional<ModRawShapedRecipe.Data> data) {
        this.width = width;
        this.height = height;
    ...
    public static ModRawShapedRecipe create(Map<Character, Ingredient> key, String... pattern) {
        return create(key, List.of(pattern));
    }

    public static ModRawShapedRecipe create(Map<Character, Ingredient> key, List<String> pattern) {
        ModRawShapedRecipe.Data data = new ModRawShapedRecipe.Data(key, pattern);
        return fromData(data).getOrThrow();
    }

    private static DataResult<ModRawShapedRecipe> fromData(ModRawShapedRecipe.Data data) {
        ...
        return !charSet.isEmpty()
                ...
                : DataResult.success(new ModRawShapedRecipe(i, j, defaultedList, Optional.of(data)));
    }
    ...
    private static ModRawShapedRecipe readFromBuf(RegistryByteBuf buf) {
        int i = buf.readVarInt();
        int j = buf.readVarInt();
        DefaultedList<Ingredient> defaultedList = DefaultedList.ofSize(i * j, Ingredient.EMPTY);
        defaultedList.replaceAll(ingredient -> Ingredient.PACKET_CODEC.decode(buf));
        return new ModRawShapedRecipe(i, j, defaultedList, Optional.empty());
    }
    ...
    public record Data(Map<Character, Ingredient> key, List<String> pattern) {
        ...
        public static final MapCodec<ModRawShapedRecipe.Data> CODEC = RecordCodecBuilder.mapCodec(
                instance -> instance.group(
                        ...
                        .apply(instance, ModRawShapedRecipe.Data::new)
        );
    }
}
